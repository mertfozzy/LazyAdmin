...
# ==============================================================================
if (-not (Test-Path $JsonPath)) { try { $AppsList = Invoke-RestMethod -Uri "https://raw.githubusercontent.com/MertRepo/lazyadmin/main/apps.json" -ErrorAction SilentlyContinue } catch {} } else { $RawJson = Get-Content $JsonPath -Raw -Encoding UTF8; $AppsList = $RawJson | ConvertFrom-Json }
if ($AppsList) {
...
$btnPause.Add_Click({ Start-Process "ms-settings:windowsupdate" }) # PowerShell ile pause zor, UI açıyoruz
